...
interface ForecastingProps {
  handleOpenInterventionModal: (
    initialData?: Partial<InterventionPlan>,
    navigate?: boolean
  ) => void;
}
...
const Forecasting: React.FC<ForecastingProps> = ({
  ...
  const processedForecastData = useMemo(() => {
    // If using integration data, return empty array (no data available yet)
    if (useIntegration) {
      return [];
    }

    const domainFiltered = regionalForecastData.filter(
      (d) => d.domain === activeDomain
    );
    const horizonMultiplier = activeHorizon === "6 Bulan" ? 1.8 : 1;

    return domainFiltered.map((item) => {
      const adjustedChange = parseFloat(
        (item.change * horizonMultiplier).toFixed(1)
      );
      const adjustedPredictedRisk = Math.max(
        0,
        Math.min(
          100,
          parseFloat((item.currentRisk + adjustedChange).toFixed(1))
        )
      );
  ...
  }, [activeDomain, activeHorizon, useIntegration]);

  const { topIncreases, topDecreases, overallTrend } = useMemo(() => {
    const sortedByChange = [...processedForecastData].sort(
      (a, b) => b.change - a.change
    );
    const overallTrendValue =
      processedForecastData.length > 0
        ? processedForecastData.reduce((acc, item) => acc + item.change, 0) /
  ...
  const dynamicChartData = useMemo(() => {
    ...
    const nationalAvgRiskOverall =
      nationalHistoricalRisk[nationalHistoricalRisk.length - 1].score;
    ...
    for (let i = 0; i < horizonMonths; i++) {
      ...
      const upper =
        currentPredicted + confidenceRange * ((i + 1) / horizonMonths);
      const lower =
        currentPredicted - confidenceRange * ((i + 1) / horizonMonths);
      predictedData.push({
  ...
  }, [activeDomain, overallTrend, activeHorizon, useIntegration]);

  const fetchInsight = async () => {
    setIsInsightLoading(true);
    setInsightError(null);
    
    // If using integration data, show message that forecasting is not available yet
    if (useIntegration) {
      setInsight("Fitur forecasting belum tersedia untuk data integration. Silakan gunakan mock data untuk melihat prediksi.");
  ...
  const handleCreatePlanFromForecast = (forecast: RegionalForecastData) => {
    const initialData: Partial<InterventionPlan> = {
      ...
      priority:
        forecast.predictedRiskLevel === "Kritis"
    ...
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-lg shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Prediksi tren risiko untuk intervensi proaktif dalam {activeHorizon}
            .
          </p>
        </div>
        <div className="flex items-center space-x-2 mt-4 sm:mt-0">
          <div className="flex rounded-md shadow-sm">
            ...
            <button
              ...
              className="bg-slate-50 border border-slate-300 text-slate-600 hover:bg-slate-100 text-sm rounded-r-lg p-2.5"
      ...
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
        <RegionalRiskTable
          ...
          onCreatePlan={handleCreatePlanFromForecast}
        />
      </div>
    </div>
  );
